...
bool AsoUtility::IsHitRect2D(const VECTOR& posA, const VECTOR& sizeA, const VECTOR& posB, const VECTOR& sizeB)
{
    return std::abs(posA.x- sizeA.x/2) < (posB.x + sizeB.x) &&
    std::abs(posB.x - sizeB.x / 2) < (posA.x + sizeA.x)&&
	std::abs(posA.y - sizeA.y / 2) < (posB.y + sizeB.y)&&
	std::abs(posB.y - sizeB.y / 2) < (posA.y + sizeA.y);
}
